...
std::vector<Entity*> Steering::relaventList(std::vector<Entity*> from, sf::Vector2f alignment, Entity* toGoTo)
{
	if (from.empty())
		return from;
	else
	{
		...
		for (int i = 0; i <= from.size() - 1 && !from.empty(); i++)
		{
			if (from[i]->getPosition() == toGoTo->getPosition())
				from.erase(from.begin() + i);
		}
		return from;
	}
...
std::vector<Entity*> Steering::collisionAvoidTo(sf::Vector2f* goalPosition, std::vector<Entity*> listOfStatics, Entity* toGoTo)
{
	//Normalizes alignemntVector in order to avoid a teleportation glitch.
	sf::Vector2f alignmentVector = normalize(*goalPosition - *agentPosition);
	...
	*agentDirection = alignmentVector;
	...
	//Removes entities that are consirered irelevant from the list.(ones that are behind the AI agent).
	detectedPoints = relaventList(detectedPoints, alignmentVector, toGoTo);
